...
export const PainPointsSection: React.FC = () => {
  ...
  return (
    <section id="pain-points" className="py-20 md:py-28 bg-[#0B0F19] relative border-t border-slate-800/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <div className="mt-6 inline-flex p-1 rounded-xl bg-slate-900 border border-slate-800 shadow-inner">
            ...
            <button
              ...
              className={`px-4 py-2 text-xs sm:text-sm font-semibold rounded-lg flex items-center gap-1.5 transition-all ${
                activeTab === 'solutions'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {PAIN_POINTS_DATA.map((item, idx) => (
            <div
              key={item.id}
              className="rounded-2xl border border-slate-800 bg-slate-900/60 overflow-hidden shadow-xl hover:border-slate-700 transition-all flex flex-col justify-between"
            >
              {/* Top part: Pain Point in Red/Dark tone */}
              {activeTab === 'both' && (
                <div className="p-6 sm:p-7 border-b border-slate-800/80 bg-gradient-to-br from-rose-950/15 via-slate-900/40 to-transparent">
              ...
              <div className="p-6 sm:p-7 bg-gradient-to-br from-emerald-950/30 via-slate-900/90 to-slate-900 border-t border-emerald-900/40 flex-1 flex flex-col justify-between">
                <div>
                ...
                <div className="pt-3 border-t border-emerald-900/30 flex items-center justify-between">
                  <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-300 bg-emerald-950/70 border border-emerald-800/60 px-3 py-1.5 rounded-full">
                    <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
                    {item.guaranteeBadge}
                  </span>
                  <span className="text-xs text-slate-400 font-medium">
                    100% зафіксовано в договорі
                  </span>
                </div>
              </div>
        ...
        {/* Bottom Banner */}
        <div className="mt-12 p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-emerald-950/40 to-slate-900 border border-emerald-500/20 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          <div>
